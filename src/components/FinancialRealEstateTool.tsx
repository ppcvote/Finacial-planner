...
export const FinancialRealEstateTool = ({ data, setData }: any) => {
  ...
  const defaultValues = {
      ...
      loanRate: 2.2,
      investReturnRate: 6,
      existingLoanBalance: 700,
      existingMonthlyPayment: 38000,
      isRefinance: false
  };

  // 2. 從 data 讀取，若無則使用預設值
  const safeData = {
    loanAmount: Number(data?.loanAmount) || defaultValues.loanAmount,
    loanTerm: Number(data?.loanTerm) || defaultValues.loanTerm,
    loanRate: Number(data?.loanRate) || defaultValues.loanRate,
    investReturnRate: Number(data?.investReturnRate) || defaultValues.investReturnRate,
    existingLoanBalance: data?.existingLoanBalance !== undefined ? Number(data.existingLoanBalance) : defaultValues.existingLoanBalance,
    existingMonthlyPayment: data?.existingMonthlyPayment !== undefined ? Number(data.existingMonthlyPayment) : defaultValues.existingMonthlyPayment,
    isRefinance: data?.isRefinance ?? defaultValues.isRefinance 
  };

  const { loanAmount, loanTerm, loanRate, investReturnRate, existingLoanBalance, existingMonthlyPayment, isRefinance } = safeData;
  ...
  const cumulativeNetIncomeTarget = isRefinance
      ? monthlySavings * loanTerm * 12 
      : monthlyCashFlowOriginal * loanTerm * 12;

  const totalWealthTargetWan = isRefinance
  ...
  // [關鍵修正] 切換模式時，同步寫入 data
  const toggleRefinance = () => {
      const newMode = !isRefinance;
      setData({
          ...safeData,
          isRefinance: newMode, // 寫入模式旗標
          existingLoanBalance: newMode ? 700 : 0, // 寫入預設餘額
          existingMonthlyPayment: newMode ? 38000 : 0 // 寫入預設月付金
      });
  };
  
  // Input Handlers (省略部分，保持原樣)
    ...
    <div className="space-y-8 animate-fade-in font-sans text-slate-800">
      ...
      <div className="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-3xl p-8 text-white shadow-lg relative overflow-hidden print-break-inside">
        ...
        <div className="relative z-10">
          ...
          <p className="text-emerald-100 text-lg opacity-90 max-w-2xl">{isRefinance ? "將不動產增值部分轉為現金，並透過投資收益降低月付金，實現資產活化與債務瘦身。" : "利用長年期低利貸款，打造不需修繕、不需找房客的「數位房地產」。讓配息自動幫您繳房貸。"}</p>
        </div>
      </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-lg border border-teal-200 p-6 h-full">
                        ...
                        {!isNegativeCashFlowOriginal ? (
                             <div className="text-center h-full flex flex-col justify-center">
                                <p className="text-slate-500 text-sm font-medium mb-1">期滿後總累積效益 (淨現金流 + 初始貸款總額)</p>
      ...
      <div className="grid md:grid-cols-2 gap-8 pt-6 border-t border-slate-200 print-break-inside">
        <div className="space-y-4 lg:col-span-1">
          <div className="flex items-center gap-2 mb-2"><RefreshCw className="text-emerald-600" size={24} /><h3 className="text-xl font-bold text-slate-800">{isRefinance ? "轉增貸致富三步" : "執行三部曲"}</h3></div>
          <div className="space-y-3">
             <div className="flex items-start gap-4 p-4 rounded-xl bg-white border border-slate-100 shadow-sm hover:border-emerald-200 transition-colors">
                ...
                <div><h4 className="font-bold text-slate-800 flex items-center gap-2">{isRefinance ? "盤點現有房貸" : "建置期 (第1年)"}</h4><p className="text-sm text-slate-600 mt-1">{isRefinance ? "檢視目前房貸餘額與已增值的房價，找出隱藏在房子裡的閒置資金。" : "透過銀行融資取得大筆資金，單筆投入穩健配息資產。就像買房出租，但省去頭期款與管理麻煩。"}</p></div>
             </div>
