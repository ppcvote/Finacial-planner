...
import { 
  Building2, Calculator, Scale, Landmark, ArrowRight, TrendingUp, CheckCircle2,
  RefreshCw, Settings, ChevronDown, ChevronUp, PiggyBank, Briefcase, ArrowDown
} from 'lucide-react';
...
export const FinancialRealEstateTool = ({ data, setData }: any) => {
  ...
  const {
      loanAmount, loanTerm, loanRate, investReturnRate, existingLoanBalance, existingMonthlyPayment,
      isRefinance, cashOutAmount,
      monthlyPayment, monthlyIncomeFull, netCashFlow, isPositiveFlow, 
      netNewMonthlyPayment, monthlySavings, totalSavingsOverTerm,
      totalOutOfPocketOriginal, totalWealthTargetWan, // 這裡要注意 utils 命名對應
      chartData
  } = result;
  
  // 為了相容原本的變數命名，做一點 mapping
  const totalWealthDisplay = isRefinance 
      ? Math.round(result.totalBenefitRefinance / 10000)
      : Math.round(result.totalBenefitStandard / 10000);
  ...
  // Refinance Mode Toggle (直接操作 data)
  // 注意：這裡我們用一個假狀態來控制 UI 顯示，但實際數據由 data 決定
  const isRefinanceMode = isRefinance; 
  ...
  const updateField = (field: string, value: number) => { 
      ...
      if (field === 'loanAmount') {
          ...
          setData({ ...data, [field]: Math.round(clampedValue) });
          ...
          setData({ ...data, [field]: Number(newValue.toFixed(1)) });
      } else {
          setData({ ...data, [field]: newValue }); 
      }
  };

  // Helper to toggle refinance mode (by setting/clearing existing loan)
  const toggleRefinance = () => {
      if (isRefinance) {
          // Turn off: clear existing loan data
          setData({ ...data, existingLoanBalance: 0, existingMonthlyPayment: 0 });
      } else {
          // Turn on: set default values
          setData({ ...data, existingLoanBalance: 700, existingMonthlyPayment: 38000 });
      }
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in font-sans text-slate-800">
        ...
        <div className="lg:col-span-4 space-y-6 print-break-inside">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 no-print">
            <h4 className="font-bold text-slate-700 mb-6 flex items-center gap-2">
              <Calculator size={20} className="text-emerald-600"/> 參數設定
            </h4>
            <div className="space-y-6">
               ...
               <div>
                   <div className="flex justify-between items-center mb-2">
                       <label className="text-sm font-medium text-slate-600">
                          {isRefinance ? "轉增貸後總額 (萬)" : "資產/貸款總額 (萬)"}
                       </label>
                       <div className="flex items-center">
                           <input type="number" min={100} max={5000} step={1} value={tempLoanAmount} onChange={handleLoanAmountInput} onBlur={finalizeLoanAmount} onKeyDown={(e) => e.key === 'Enter' && finalizeLoanAmount()} className="w-20 text-right bg-transparent border-none p-0 font-mono font-bold text-emerald-600 text-lg focus:ring-0 focus:border-emerald-500 focus:bg-emerald-50/50 rounded"/>
               ...
               <div>
                 <div className="flex justify-between mb-2">
                   ...
                   <input type="range" min={10} max={40} step={1} value={loanTerm} onChange={(e) => updateField('loanTerm', Number(e.target.value))} className="w-full h-2 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-teal-600 hover:accent-teal-700 transition-all" />
               </div>
               ...
               {/* --- 進階設定 (轉增貸) --- */}
               <button onClick={() => setShowAdvanced(!showAdvanced)} className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all duration-200 ${showAdvanced || isRefinance ? 'bg-orange-50 border-orange-200 text-orange-800' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`}>
                  <div className="flex items-center gap-2 font-bold text-sm"><Settings size={16} /> 進階設定 (轉增貸試算)</div>
                  {showAdvanced ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
               </button>

               {(showAdvanced || isRefinance) && (
                 <div className="space-y-4 animate-in slide-in-from-top-2 duration-300 pt-2 border-t border-orange-100">
                    <div className="flex items-center justify-between">
                        ...
                        <button onClick={toggleRefinance} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${isRefinance ? 'bg-orange-500' : 'bg-slate-300'}`}>
                            <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${isRefinance ? 'translate-x-6' : 'translate-x-1'}`} />
                        </button>
                    </div>
                    ...
                    {isRefinance && (
                        <div className="space-y-4 bg-orange-50/50 p-4 rounded-xl border border-orange-100">
                            <div>
                                ...
                                <input type="range" min={0} max={loanAmount} step={1} value={existingLoanBalance} onChange={(e) => updateField('existingLoanBalance', Number(e.target.value))} className="w-full h-1.5 bg-orange-200 rounded-lg appearance-none cursor-pointer accent-orange-500" />
                            </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-[400px] print-break-inside relative">
             <div className="flex justify-between items-center mb-4 pl-2 border-l-4 border-emerald-500">
                <h4 className="font-bold text-slate-700">{isRefinance ? "轉增貸效益模擬" : "資產淨值成長模擬"}</h4>
            ...
            <ResponsiveContainer width="100%" height="90%">
              <ComposedChart data={chartData} margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
                ...
                {isRefinance ? (
                   ...
                   <>
                     <Area type="monotone" name="總資產價值" dataKey="總資產" stroke="#10b981" fill="url(#colorWealth)" strokeWidth={3} />
               ...
               <div className="md:col-span-1 bg-white rounded-2xl shadow border border-orange-200 p-6 print-break-inside flex flex-col justify-between">
                  ...
                  <div className="flex items-center justify-between gap-2 mb-4">
                      <div className="text-center w-1/3">
                          ...
                          <div className="text-xl font-bold text-slate-400 line-through">${Math.round(existingMonthlyPayment).toLocaleString()}</div>
                      </div>
                      <ArrowRight size={24} className="text-orange-500" />
                      <div className="text-center w-1/3">
                          <div className="text-xs text-slate-500 mb-1">整合後月付</div>
                          <div className="text-2xl font-black text-emerald-600">${Math.round(netNewMonthlyPayment).toLocaleString()}</div>
                      </div>
                  </div>
                  <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100 text-center space-y-3">
                       <div>
                            ...
                            <p className="text-4xl font-black text-emerald-600 font-mono">${Math.round(monthlySavings).toLocaleString()}</p>
                       </div>
                       <div className="border-t border-emerald-200/50 pt-2">
                            <p className="text-xs text-emerald-700 font-bold mb-0.5">{loanTerm}年總計省下</p>
                            <p className="text-xl font-black text-emerald-700 font-mono">${Math.round(totalSavingsOverTerm / 10000).toLocaleString()} 萬</p>
                       </div>
                  </div>
               </div>
               ...
               <div className="md:col-span-1 bg-white rounded-2xl shadow border border-slate-200 p-6 print-break-inside space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
                      <div className="bg-slate-50 p-4 rounded-xl text-center border border-slate-100 flex flex-col items-center justify-center">
                          ...
                          <Building2 className="text-slate-400 mb-2" size={32} />
                      ...
                      <div className="bg-orange-50 p-4 rounded-xl text-center border border-orange-100 flex flex-col items-center justify-center relative overflow-hidden">
                          ...
                          <div className="text-xl font-black text-orange-600 mt-1 font-mono">${cashOutAmount} 萬</div>
                      </div>
                  </div>
               </div>
            </>
            ...
            <>
                <div className="md:col-span-1 bg-white rounded-2xl shadow border border-slate-200 p-6 print-break-inside">
                    ...
                    <div className="space-y-4 bg-slate-50 p-5 rounded-xl border border-slate-100">
                        ...
                        <div className="flex justify-between items-center text-sm"><span className="text-slate-600 font-medium">2. 扣除貸款支出</span><span className="font-mono text-red-500 font-bold">-${Math.round(monthlyPayment).toLocaleString()}</span></div>
                        <div className="border-t border-slate-200 my-2"></div>
                        
                        {!isPositiveFlow ? (
                        <div className="text-center animate-pulse-soft">
                            <div className="text-xs text-slate-400 mb-1">每月需自行負擔</div>
                            <div className="text-4xl font-black text-red-500 font-mono">-${Math.abs(Math.round(netCashFlow)).toLocaleString()}</div>
                        ...
                        <div className="text-center">
                            ...
                            <div className="text-4xl font-black text-emerald-600 font-mono">+${Math.round(netCashFlow).toLocaleString()}</div>
                ...
                <div className="md:col-span-1 print-break-inside space-y-6">
                    <div className="bg-white rounded-2xl shadow-lg border border-teal-200 p-6 h-full">
                        <h3 className="text-xl font-bold text-teal-700 mb-2 flex items-center gap-2">
                            <TrendingUp size={24} /> {isPositiveFlow ? `總貸款期 (${loanTerm}年) 累積總效益` : "槓桿交換效益"}
                        </h3>
                        
                        {!isPositiveFlow ? (
                             <div className="text-center h-full flex flex-col justify-center space-y-4">
                                 <div>
                                    <p className="text-slate-500 text-sm font-medium mb-1">總成本 (分期付款)</p>
                                    <p className="text-3xl font-black text-red-500 font-mono">${Math.round(totalOutOfPocketOriginal/10000).toLocaleString()} 萬</p>
                                 </div>
                                 <div className="relative">
                                    <div className="absolute inset-0 flex items-center justify-center"><div className="w-full border-t border-slate-200"></div></div>
                                    <div className="relative bg-white px-2 flex justify-center"><span className="text-xs text-slate-400 bg-slate-100 px-3 py-1 rounded-full flex items-center gap-1"><ArrowDown size={12}/> 交換 <ArrowDown size={12}/></span></div>
                                 </div>
                                 <div>
                                    <p className="text-slate-500 text-sm font-medium mb-1">期滿擁有一棟房 (資產)</p>
                                    <p className="text-4xl font-black text-emerald-600 font-mono">${loanAmount.toLocaleString()} 萬</p>
                                 </div>
                             </div>
                        ) : (
                             <div className="text-center h-full flex flex-col justify-center">
                                <p className="text-slate-500 text-sm font-medium mb-1">期滿後總累積效益 (淨現金流 + 資產)</p>
                                <p className="text-5xl font-black text-emerald-600 font-mono">${totalWealthDisplay.toLocaleString()} 萬</p>
                            </div>
                        )}
                    </div>
                </div>
      ...
      <div className="grid md:grid-cols-2 gap-8 pt-6 border-t border-slate-200 print-break-inside">
         {/* ... (省略重複的靜態內容以節省空間，請保持原檔案下半部不變，或如果需要我也可以完整列出) */}
         {/* 為了完整性，這裡還是附上底部 */}
         <div className="space-y-4 lg:col-span-1">
          <div className="flex items-center gap-2 mb-2">
             <RefreshCw className="text-emerald-600" size={24} />
             <h3 className="text-xl font-bold text-slate-800">{isRefinance ? "轉增貸致富三步" : "執行三部曲"}</h3>
          </div>
          <div className="space-y-3">
             <div className="flex items-start gap-4 p-4 rounded-xl bg-white border border-slate-100 shadow-sm hover:border-emerald-200 transition-colors">
                <div className="mt-1 min-w-[3rem] h-12 rounded-xl bg-emerald-50 text-emerald-600 flex flex-col items-center justify-center font-bold text-xs"><span className="text-lg">01</span><span>{isRefinance ? "盤點" : "建置"}</span></div>
                <div>
                   <h4 className="font-bold text-slate-800 flex items-center gap-2">{isRefinance ? "盤點現有房貸" : "建置期 (第1年)"}</h4>
                   <p className="text-sm text-slate-600 mt-1">{isRefinance ? "檢視目前房貸餘額與已增值的房價，找出隱藏在房子裡的閒置資金。" : "透過銀行融資取得大筆資金，單筆投入穩健配息資產。就像買房出租，但省去頭期款與管理麻煩。"}</p>
                </div>
             </div>
             {/* ... Step 2 & 3 ... */}
             <div className="flex items-start gap-4 p-4 rounded-xl bg-white border border-slate-100 shadow-sm hover:border-teal-200 transition-colors">
                <div className="mt-1 min-w-[3rem] h-12 rounded-xl bg-teal-50 text-teal-600 flex flex-col items-center justify-center font-bold text-xs"><span className="text-lg">02</span><span>{isRefinance ? "增貸" : "持守"}</span></div>
                <div><h4 className="font-bold text-slate-800 flex items-center gap-2">{isRefinance ? "轉貸增值" : "持守期 (第2年起)"}</h4><p className="text-sm text-slate-600 mt-1">{isRefinance ? "透過轉貸將增值部分套現，同時爭取更長的年限與更低的利率，降低還款壓力。" : "讓資產產生的配息自動償還貸款本息。您只需補貼少許差額(甚至有找)，時間是您最好的朋友。"}</p></div>
             </div>
             <div className="flex items-start gap-4 p-4 rounded-xl bg-white border border-slate-100 shadow-sm hover:border-green-200 transition-colors">
                <div className="mt-1 min-w-[3rem] h-12 rounded-xl bg-green-50 text-green-600 flex flex-col items-center justify-center font-bold text-xs"><span className="text-lg">03</span><span>{isRefinance ? "套利" : "自由"}</span></div>
                <div><h4 className="font-bold text-slate-800 flex items-center gap-2">{isRefinance ? "配息減壓" : "自由期 (期滿)"}</h4><p className="text-sm text-slate-600 mt-1">{isRefinance ? "將增貸資金投入穩健標的，利用產生的配息來支付房貸，實現月付金大瘦身。" : "貸款完全清償。此刻起，這筆千萬資產與每月的配息收入完全屬於您，成為真正的被動收入。"}</p></div>
             </div>
          </div>
          <div className="mt-6 p-4 bg-slate-800 rounded-xl text-center shadow-lg">
             <p className="text-slate-300 italic text-sm">{isRefinance ? "「房子不只是拿來住的，更是您的提款機。別讓您的資產在牆壁裡睡覺。」" : "「富人買資產，窮人買負債，中產階級買他們以為是資產的負債。金融房產，是真正的資產。」"}</p>
           </div>
        </div>
