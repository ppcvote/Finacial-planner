...
const SuperActiveReport = ({ data }: { data: any }) => {
  ...
  return (
    // [調整]: print:space-y-4 (維持適當間距，避免過鬆)
    <div className="font-sans text-slate-800 space-y-5 print:space-y-4 relative text-sm print:text-xs">
      ...
      {/* 2. 核心分析：人生自由度量表 (修正定位邏輯) */}
      <div className="relative z-10 bg-slate-50 rounded-xl p-4 border border-slate-200 print-break-inside print:p-3">
          ...
          {/* [FIX]: 建立獨立的 relative wrapper，不帶 padding，確保寬度基準與定位基準一致 */}
          <div className="relative w-full mt-4 mb-2 print:mt-3">
              {/* 進度條背景 */}
              ...
              {/* 標籤指針 - 自由期長度 */}
              <div 
                className="absolute top-5 transform -translate-x-1/2 whitespace-nowrap"
                style={{left: `${freedomCenterPercent}%`}}
              >
                  <p className="text-fuchsia-600 font-bold text-xs flex items-center justify-center gap-1 print:text-[10px]">
      ...
      <div className="relative z-10 space-y-2 print-break-inside">
          ...
          {/* [調整]: 高度 200px - 確保頁面不溢出 */}
          <div className="h-[240px] w-full border border-slate-100 rounded-xl p-3 bg-white shadow-sm print:h-[200px] print:p-2">
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-3 print:p-3 print-break-inside">
          ...
          <ul className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-[10px] text-slate-600">
      ...
      {/* 7. 顧問總結 (Footer) */}
      <div className="relative z-10 bg-slate-50 p-3 rounded-xl border-l-4 border-violet-500 print-break-inside print:p-2 print:mt-3">
