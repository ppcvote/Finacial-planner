...
const SuperActiveReport = ({ data }: { data: any }) => {
  ...
  return (
    // [調整]: print:space-y-8 (大幅拉開間距), print:text-sm (放大字體)
    <div className="font-sans text-slate-800 space-y-8 print:space-y-8 relative text-base print:text-sm">
      ...
      <div className="relative z-10 bg-slate-50 rounded-3xl p-6 border border-slate-200 print-break-inside print:p-6">
          ...
          <div className="w-full">
              {/* 進度條 (Flex Container) */}
              <div className="w-full h-8 bg-slate-200 rounded-full overflow-hidden flex shadow-inner border border-slate-300/50">
                  {/* 工作區 */}
                  <div 
                    className="h-full bg-slate-600 flex items-center justify-center text-xs text-white font-bold tracking-widest border-r border-white/20" 
                  ...
                  {/* 自由區 */}
                  <div 
                    className="h-full bg-gradient-to-r from-violet-500 to-fuchsia-500 flex items-center justify-center text-xs text-white font-bold tracking-widest" 
              ...
              {/* 下方標示區 (Flex Container 對齊上方進度條) */}
              <div className="flex w-full mt-2">
                  
                  {/* 工作區下方 - 對齊右側邊界 (分隔線) */}
                  <div style={{width: `${workRatio * 100}%`}} className="relative flex justify-end">
                      {/* 分隔線指標 (絕對定位於右邊界) */}
                      <div className="absolute right-0 transform translate-x-1/2 -top-12 flex flex-col items-center z-20">
                          <div className="bg-slate-800 text-white text-xs px-2 py-1 rounded mb-1 whitespace-nowrap font-bold shadow-md">
                              {activeYears} 年
                          </div>
                  ...
                  {/* 自由區下方 - 自然置中 */}
                  <div style={{width: `${freedomRatio * 100}%`}} className="flex justify-center pt-2">
                      <div className="bg-white border border-fuchsia-200 rounded-full px-4 py-1.5 shadow-sm flex items-center gap-2">
                          ...
                          <p className="text-fuchsia-600 font-bold text-sm print:text-sm whitespace-nowrap">
                              恭喜！贖回 <span className="text-lg mx-1">{totalYears - activeYears}</span> 年人生
                          </p>
                      </div>
                  </div>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-violet-200 p-0 shadow-sm relative overflow-hidden print:border-slate-300">
              ...
              <div className="p-6 space-y-4 relative print:p-5 print:space-y-3">
                  ...
                  <div className="flex justify-between items-end border-b border-slate-100 pb-2">
                      ...
                      <span className="font-bold text-slate-700 italic text-base">未來的您 (Future You)</span>
                  </div>
      ...
      {/* 5. 資產趨勢圖 (Active vs Passive) - 高度加大至 320px */}
      <div className="relative z-10 space-y-4 print-break-inside print:space-y-3">
          ...
          {/* [調整]: 高度 320px - 填滿頁面 */}
          <div className="h-[350px] w-full border border-slate-100 rounded-2xl p-5 bg-white shadow-sm print:h-[320px] print:p-4">
              <ResponsiveContainer width="100%" height="100%">
