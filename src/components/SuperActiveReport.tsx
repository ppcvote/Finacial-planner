...
const SuperActiveReport = ({ data }: { data: any }) => {
  ...
  return (
    // [調整]: space-y-6 / print:space-y-6 (拉大間距，填滿版面)
    // [調整]: text-base / print:text-sm (字體放大，閱讀舒適)
    <div className="font-sans text-slate-800 space-y-6 print:space-y-6 relative text-base print:text-sm">
      ...
      {/* 1. Header (高度舒適) */}
      <div className="relative z-10 flex items-center justify-between border-b-2 border-fuchsia-100 pb-5 print:pb-5 print-break-inside bg-white/50 backdrop-blur-sm">
         <div className="flex items-center gap-5">
             ...
             <div>
                 <span className="text-xs font-bold text-fuchsia-600 tracking-widest uppercase block mb-1 print:text-xs">FIRE Movement Plan</span>
                 <h1 className="text-3xl font-black text-slate-900 mb-1 print:text-3xl leading-none">超積極存錢法</h1>
      ...
      {/* 2. 核心分析：人生自由度量表 (Flexbox 結構定位法 - 絕對穩固) */}
      <div className="relative z-10 bg-slate-50 rounded-2xl p-6 border border-slate-200 print-break-inside print:p-5">
          <div className="flex items-center justify-between mb-5 print:mb-5">
          ...
          <div className="w-full pt-4">
              {/* 進度條容器 */}
              <div className="w-full h-6 bg-slate-200 rounded-full flex shadow-inner border border-slate-300/50 relative overflow-visible">
                  ...
                  >
                      ...
                      {/* [核心修正]: 分隔線指標直接掛在 WORK 區塊的「內部右側」 */}
                      {/* right-0: 緊貼右邊界 (交界處) */}
                      {/* translate-x-1/2: 往右移一半，正好騎在線上 */}
                      ...
                      <div className="absolute right-0 -top-10 translate-x-1/2 flex flex-col items-center z-30">
                          <div className="bg-slate-800 text-white text-xs px-2 py-1 rounded mb-1 whitespace-nowrap font-bold shadow-md print:text-xs">
                              {activeYears} 年
                          </div>
              ...
              <div className="flex w-full mt-4">
                  ...
                  <div style={{width: `${freedomRatio * 100}%`}} className="flex justify-center">
                      <div className="bg-white border border-fuchsia-200 rounded-full px-4 py-2 shadow-sm flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 gap-6 print:gap-6 print-break-inside">
          ...
          <div className="bg-white rounded-2xl border border-violet-200 p-0 shadow-sm relative overflow-hidden print:border-slate-300">
              ...
              <div className="p-6 space-y-3 relative print:p-5 print:space-y-3">
                  ...
                  <div className="flex justify-between items-end">
                      <div className="text-3xl font-black text-violet-600 font-mono print:text-slate-800 print:text-3xl">
                          ${monthlyPassiveIncome.toLocaleString()}
                      </div>
                      <span className="text-xs text-slate-400 mb-1 print:text-xs">/ 月 (Monthly)</span>
                  </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm relative flex items-center justify-between print:p-5">
              <div className="w-[55%] z-10 pl-2">
                  <h4 className="font-bold text-slate-700 mb-3 text-sm print:text-sm">財富組成分析</h4>
                  <div className="space-y-3">
                      <div className="flex items-start gap-2">
                          <div className="w-2.5 h-2.5 rounded-full bg-violet-400 mt-1"></div>
                          <div>
                              <p className="text-xs text-slate-500">本金 (勞力)</p>
                              <p className="text-sm font-bold text-slate-700 print:text-sm">${Math.round(totalPrincipalActive/10000).toLocaleString()} 萬</p>
                          </div>
                      </div>
                      <div className="flex items-start gap-2">
              ...
              <div className="w-[45%] h-[120px] relative print:h-[120px]">
      ...
      <div className="relative z-10 print-break-inside">
          ...
          <div className="grid grid-cols-3 gap-4 print:gap-4">
              {[
                  ...
                  <div key={i} className={`rounded-2xl border p-4 text-center ${p.color} print:p-4 flex flex-col justify-between h-full`}>
                      <div>
                          <p className="text-xs opacity-60 mb-1 font-bold uppercase print:text-xs">{p.sub}</p>
                          <h4 className="font-bold text-base mb-1.5 print:text-base">{p.name}</h4>
                          <p className="text-xs opacity-90 leading-relaxed print:text-xs">{p.desc}</p>
                      </div>
                  </div>
              ))}
          </div>
      ...
      <div className="relative z-10 space-y-4 print-break-inside print:space-y-4">
          <div className="flex items-center justify-between mb-2">
              <h2 className="text-xl font-bold text-slate-700 flex items-center gap-2 print:text-lg">
                  <TrendingUp size={24} className="text-violet-600 print:w-5 print:h-5"/>
                  資產成長模擬
              </h2>
              <div className="text-xs text-slate-500 font-medium bg-slate-100 px-3 py-1 rounded print:text-xs">
                  {activeYears}年積極 / {totalYears}年總期程
              </div>
          </div>
          
          {/* [調整]: 高度 300px，這會有效填補第二頁的下方空白 */}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 print:p-5 print-break-inside">
          <h3 className="font-bold text-slate-700 text-base mb-3 flex items-center gap-2 print:text-base">
              <Zap size={20} className="text-yellow-500"/> 專案執行亮點
          </h3>
          <ul className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm text-slate-600 print:text-sm">
              <li className="flex items-start gap-2"><CheckCircle2 size={16} className="text-fuchsia-500 mt-0.5 shrink-0"/> <span>本金極省：相比傳統，少付了 ${(Math.round(savedPrincipal/10000)).toLocaleString()} 萬本金。</span></li>
              <li className="flex items-start gap-2"><CheckCircle2 size={16} className="text-fuchsia-500 mt-0.5 shrink-0"/> <span>縮短工時：只需努力 {activeYears} 年，提早贖回人生自由。</span></li>
              <li className="flex items-start gap-2"><CheckCircle2 size={16} className="text-fuchsia-500 mt-0.5 shrink-0"/> <span>抗通膨：透過長期投資複利，避免存款越存越薄。</span></li>
              <li className="flex items-start gap-2"><CheckCircle2 size={16} className="text-fuchsia-500 mt-0.5 shrink-0"/> <span>選擇權：工作是為了興趣而非生存，擁有說「不」的權利。</span></li>
          </ul>
      </div>

      {/* 7. 顧問總結 (Footer) */}
      <div className="relative z-10 bg-slate-50 p-5 rounded-2xl border-l-4 border-violet-500 print-break-inside print:p-5 print:mt-6">
          <div className="flex gap-4">
               <Quote className="text-violet-300 shrink-0" size={28} />
               <div>
                   <h3 className="font-bold text-slate-800 text-base mb-1 print:text-base">顧問觀點</h3>
