...
const SuperActiveReport = ({ data }: { data: any }) => {
  ...
  return (
    // [調整]: space-y-5 -> print:space-y-3 (緊縮)
    <div className="font-sans text-slate-800 space-y-5 print:space-y-3 relative text-sm print:text-xs">
      ...
      {/* 2. 核心分析：人生自由度量表 (修復顯示錯誤 & 緊湊化) */}
      <div className="relative z-10 bg-slate-50 rounded-xl p-4 border border-slate-200 print-break-inside print:p-2">
          ...
          <div className="relative pt-5 pb-1 px-1 print:pt-4">
              {/* 進度條背景 */}
              ...
              >
                  <div className="bg-slate-700 text-white text-[9px] px-1.5 py-0.5 rounded mb-0.5 whitespace-nowrap font-bold shadow-sm">
                      {activeYears} 年
                  </div>
                  <div className="w-px h-6 bg-slate-700/50"></div>
              </div>

              {/* [Fix] 標籤指針 - 自由期長度 (使用絕對定位 + translate 確保置中) */}
              <div 
                className="absolute top-7 transform -translate-x-1/2 w-full text-center print:top-6"
      ...
      <div className="grid grid-cols-2 gap-3 print:gap-3 print-break-inside">
          ...
          <div className="bg-white rounded-xl border border-violet-200 p-0 shadow-sm relative overflow-hidden print:border-slate-300">
              ...
              {/* 支票內容 */}
              <div className="p-3 space-y-2 relative print:p-2">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-3 shadow-sm relative flex items-center justify-between print:p-2">
              <div className="w-[55%] z-10">
                  <h4 className="font-bold text-slate-700 mb-1 text-xs print:text-[10px]">財富組成分析</h4>
      ...
      <div className="relative z-10 print-break-inside">
          ...
          <div className="grid grid-cols-3 gap-2 print:gap-2">
              {[
                  ...
                  <div key={i} className={`rounded-xl border p-2 text-center ${p.color} print:p-2 flex flex-col justify-between h-full`}>
                      <div>
                          <p className="text-[9px] opacity-60 mb-0.5 font-bold uppercase">{p.sub}</p>
                          <h4 className="font-bold text-xs mb-1 print:text-[10px]">{p.name}</h4>
                          <p className="text-[10px] opacity-90 leading-relaxed print:text-[9px]">{p.desc}</p>
                      </div>
                  </div>
              ))}
          </div>
      ...
      {/* 5. 資產趨勢圖 (Active vs Passive) - 高度縮減 */}
      <div className="relative z-10 space-y-2 print-break-inside">
          ...
          {/* [調整]: print:h-[180px] - 關鍵瘦身點 */}
          <div className="h-[220px] w-full border border-slate-100 rounded-xl p-3 bg-white shadow-sm print:h-[180px] print:p-2">
              <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={fullChartData} margin={{ top: 15, right: 10, left: 0, bottom: 5 }}>
                      ...
                      <YAxis unit="萬" tick={{fontSize: 9}} width={35} tickLine={false} axisLine={false}/>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-3 print:p-2 print-break-inside">
          ...
          <ul className="grid grid-cols-2 gap-x-2 gap-y-1 text-[10px] text-slate-600 print:text-[9px]">
              <li className="flex items-start gap-1"><CheckCircle2 size={10} className="text-fuchsia-500 mt-0.5 shrink-0"/> <span>本金極省：相比傳統，少付了 ${(Math.round(savedPrincipal/10000)).toLocaleString()} 萬本金。</span></li>
              <li className="flex items-start gap-1"><CheckCircle2 size={10} className="text-fuchsia-500 mt-0.5 shrink-0"/> <span>縮短工時：只需努力 {activeYears} 年，提早贖回人生自由。</span></li>
              ...
              <li className="flex items-start gap-1"><CheckCircle2 size={10} className="text-fuchsia-500 mt-0.5 shrink-0"/> <span>選擇權：工作是為了興趣而非生存，擁有說「不」的權利。</span></li>
          </ul>
      </div>
