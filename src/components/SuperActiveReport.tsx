    ...
    <div className="font-sans text-slate-800 space-y-4 print:space-y-2 relative text-sm print:text-xs">
      ...
      <div className="relative z-10 bg-slate-50 rounded-xl p-3 border border-slate-200 print-break-inside print:p-2">
          ...
          <div className="w-full mt-2">
              {/* 進度條 (Flex Container) */}
              <div className="flex w-full h-5 rounded-full overflow-hidden shadow-sm border border-slate-300/50">
                  <div 
                    className="bg-slate-600 flex items-center justify-center text-[9px] text-white font-bold tracking-wider print:text-[8px]" 
                    style={{width: `${workRatio * 100}%`}}
                  >
                      WORK
                  </div>
                  <div 
                    className="bg-gradient-to-r from-violet-500 to-fuchsia-500 flex items-center justify-center text-[9px] text-white font-bold tracking-wider print:text-[8px]" 
              ...
              {/* 下方對照資訊 (Flex Container - 絕對對齊上方色塊) */}
              <div className="flex w-full mt-1">
                  <div style={{width: `${workRatio * 100}%`}} className="text-center border-r border-slate-300 pr-1">
                      <p className="text-[9px] text-slate-500 font-bold mb-0.5">努力 {activeYears} 年</p>
                  </div>
                  <div style={{width: `${freedomRatio * 100}%`}} className="text-center pl-1">
                      <p className="text-[9px] text-fuchsia-600 font-bold mb-0.5">享受 {totalYears - activeYears} 年</p>
                  </div>
              </div>

              {/* 獨立醒目提示框 (不再依賴動態定位) */}
              <div className="mt-2 bg-white border border-fuchsia-200 rounded-lg p-1.5 text-center shadow-sm">
                  <p className="text-fuchsia-600 font-bold text-xs flex items-center justify-center gap-1 print:text-[10px]">
                      <Star size={12} fill="currentColor"/> 
                      恭喜！您成功贖回了 <span className="text-sm underline print:text-xs">{totalYears - activeYears}</span> 年的人生使用權
                  </p>
              </div>
          </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 print:gap-2 print-break-inside">
          ...
          <div className="bg-white rounded-xl border border-violet-200 p-0 shadow-sm relative overflow-hidden print:border-slate-300">
              <div className="bg-violet-50/50 p-2 border-b border-violet-100 flex justify-between items-center print:bg-slate-50 print:border-slate-200">
                  <span className="text-[9px] font-bold text-violet-400 tracking-widest print:text-slate-400">PASSIVE INCOME</span>
                  <span className="text-[9px] font-mono text-slate-400">NO. 000{activeYears}888</span>
              </div>
              <div className="p-3 space-y-1 relative print:p-2 print:space-y-1">
      ...
      <div className="relative z-10 space-y-2 print-break-inside">
          ...
          {/* [調整]: 高度 180px - 確保版面不溢出 */}
          <div className="h-[200px] w-full border border-slate-100 rounded-xl p-3 bg-white shadow-sm print:h-[180px] print:p-2">
              <ResponsiveContainer width="100%" height="100%">
      ...
      {/* 7. 顧問總結 (Footer) - 緊湊 */}
      <div className="relative z-10 bg-slate-50 p-2 rounded-xl border-l-4 border-violet-500 print-break-inside print:p-2 print:mt-2">
          <div className="flex gap-2">
               <Quote className="text-violet-300 shrink-0" size={18} />
               <div>
